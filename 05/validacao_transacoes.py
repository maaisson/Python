...
with conn.connect() as connection:
    t = text('SELECT category as categoria, status, sum(amount) as soma FROM transactions Group By status, categoria ORDER BY categoria, status;')    
...
with conn.connect() as connection:
    t = text('SELECT status, sum(amount) as soma FROM transactions Group By status ORDER BY status;')    
    result = connection.execute(t)	
    rows = result.fetchall()
    columns = result.keys()
